#!/usr/bin/env node
/**
 * Fetch contiguous first-author posts for HF News editions and save to editions/<edition>/posts.json
 *
 * Usage:
 *   node scripts/fetch_posts.js --start <StartEditionNumber> --end <EndEditionNumber> --token <APIToken>
 *
 * Notes:
 * - Reads editions and links from <repo>/editions.csv (Extracted,Edition,Link,Name,Note)
 * - Skips editions without a link or without a tid in the link and logs "Edition <n> - no tid found"
 * - Skips editions that already have editions/<edition>/posts.json
...
/** Repository root (parent of `scripts/`), not `process.cwd()`. */
...
function readCsv(filePath) {
  ...
  // Expect header at line 0: Extracted,Edition,Link,Name,Note
  const rows = [];
  for (let i = 1; i < lines.length; i++) {
    const [, edition, link] = parseCsvLineFirstN(lines[i], 3);
...
async function main() {
  ...
  const editionsCsvPath = path.join(PROJECT_ROOT, "editions.csv");
  ...
  for (let ed = startNum; ed <= endNum; ed++) {
    ...
    const editionDir = path.join(PROJECT_ROOT, "editions", editionKey);
